...
async function login(ctx) {
  ...
  if (body.type() === "form") {
    ...
    // 檢查是否有符合條件的用戶
    if (dbUsers.length === 0) {
      ctx.response.body = render.fail(); // 顯示錯誤頁面，提示用戶不存在
      return;
    }

    var dbUser = dbUsers[0];
    if (dbUser.password === user.password) {
      ctx.state.session.set('user', dbUser); // 將從資料庫查詢的用戶存入 session
      console.log('session.user=', await ctx.state.session.get('user'));
      ctx.response.redirect('/'); // 登錄成功後跳轉到首頁
    } else {
      ctx.response.body = render.fail(); // 密碼錯誤，顯示錯誤頁面
    }
  }
}
...
async function listUserPosts(ctx) {
    const username = ctx.params.user; // 從 URL 中提取使用者名稱
    const posts = postQuery(`SELECT id, username, title, body FROM posts WHERE username='${username}'`); // 查詢貼文
    const user = await ctx.state.session.get('user'); // 確認目前登入的使用者
    if (posts.length === 0) {
      ctx.response.body = render.fail(); // 如果沒有找到貼文
    } else {
      ctx.response.body = render.list(posts, user); // 顯示貼文列表
    }
  }
